...
const ComputersCanvas = () => {
  ...
  const [position, setPosition] = useState([0, -50, 0]);

  const transition = useTransition(position, {
    config:  { duration: 1000 },
    from: { position: [0, -50, 0] },
    enter: { position },
  });


  const handleLeftButtonClick = () => {
    const endPosition = [position[0], position[1] - 2, position[2]];
  ...
  const animatedComputers = animated(Computers)
    ...
    return (
      <mesh>
        ...
        <primitive 
          ...
          scale={isMobile ? 0.50 : 6}
          position={isMobile ? [0, 0.8, 0] : position}
          rotation={[0, 0, 0]}
        />
      </mesh>
    )
  ...
  return(
    ...
    >
      <Suspense fallback={<CanvasLoader/>}>
        ...
        /> 
         {transition((props, item, _, index) => (
           <animated.group
           position={props.position} 
           key={index}>
            <animatedComputers isMobile={isMobile} />
            </animated.group>
            ))}
      </Suspense>
      <Preload all />
    </Canvas>
    <div className="my-5" style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%" }}>
        <button className='border rounded px-3 py-1 mx-5' onClick={handleLeftButtonClick}>Left</button>
        <button className='border rounded px-3 py-1' onClick={handleRightButtonClick}>Right</button>
      </div>
    </>
